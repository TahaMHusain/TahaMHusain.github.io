const MAX_BUFFERED_AMOUNT = 65536,
  ICECOMPLETE_TIMEOUT = 5e3,
  CHANNEL_CLOSING_TIMEOUT = 5e3;
function randombytes(e) {
  const t = new Uint8Array(e);
  for (let s = 0; s < e; s++) t[s] = (256 * Math.random()) | 0;
  return t;
}
...
class Peer {
  constructor(e = {}) {
    if (
      ...
      (this.channelName = e.initiator
        ? e.channelName || randombytes(20).toString("hex")
        : null),
      (this.initiator = e.initiator || !1),
      (this.channelConfig = e.channelConfig || Peer.channelConfig),
      (this.channelNegotiated = this.channelConfig.negotiated),
      (this.config = Object.assign({}, Peer.config, e.config)),
      ...
      (this._wrtc =
        e.wrtc && "object" == typeof e.wrtc ? e.wrtc : getBrowserRTC()),
      !this._wrtc)
    )
    ...
    try {
      ...
      return void this.destroy(errCode(e, "ERR_PC_CONSTRUCTOR"));
    }
    (this._isReactNativeWebrtc = "number" == typeof this._pc._peerConnectionId),
      ...
      "object" == typeof this._pc.peerIdentity &&
        this._pc.peerIdentity.catch((e) => {
          this.destroy(errCode(e, "ERR_PC_PEER_IDENTITY"));
        }),
      ...
      this._debug("signal()"),
        ...
        e.sdp &&
          this._pc
            ...
            .catch((e) => {
              this.destroy(errCode(e, "ERR_SET_REMOTE_DESCRIPTION"));
            }),
        e.sdp ||
          e.candidate ||
          e.renegotiate ||
  ...
  _addIceCandidate(e) {
    ...
    this._pc.addIceCandidate(t).catch((e) => {
      !t.address || t.address.endsWith(".local")
        ...
        : this.destroy(errCode(e, "ERR_ADD_ICE_CANDIDATE"));
    });
  }
  send(e) {
    if (!this.destroying) {
    ...
    if (!this.destroying) {
      ...
      if ((this._debug("addTransceiver()"), this.initiator))
        try {
          ...
          this.destroy(errCode(e, "ERR_ADD_TRANSCEIVER"));
        }
      else
        this.emit("signal", {
  ...
  removeTrack(e, t) {
    ...
    try {
      ...
      "NS_ERROR_UNEXPECTED" === e.name
        ...
        : this.destroy(errCode(e, "ERR_REMOVE_TRACK"));
    }
    this._needsNegotiation();
  }
  removeStream(e) {
  ...
  _setupData(e) {
    ...
    (this._channel = e.channel),
      ...
      (this._channel.onerror = (e) => {
        this.destroy(errCode(e, "ERR_DATA_CHANNEL"));
      });
    let t = !1;
      ...
      this._pc
        ...
        .then((e) => {
          ...
          this._pc
            ...
            .catch((e) => {
              this.destroy(errCode(e, "ERR_SET_LOCAL_DESCRIPTION"));
            });
        })
        .catch((e) => {
          this.destroy(errCode(e, "ERR_CREATE_OFFER"));
        });
  }
  ...
  _createAnswer() {
    this.destroyed ||
      this._pc
        ...
        .catch((e) => {
          this.destroy(errCode(e, "ERR_CREATE_ANSWER"));
        });
  }
          ...
          if (
            ...
            if (
              ...
              try {
                ...
                return this.destroy(errCode(t, "ERR_DATA_CHANNEL"));
              }
              (this._chunk = null),
  ...
  _debug(...e) {
    this._doDebug && ((e[0] = "[" + this._id + "] " + e[0]), console.log(...e));
  }
  on(e, t) {
    const s = this._map;
  ...
  off(e, t) {
    const s = this._map,
      i = s.get(e);
    i && (i.delete(t), 0 === i.size && s.delete(e));
  }
  once(e, t) {
    const s = (...i) => {
      this.off(e, s), t(...i);
...
const charSet =
  ...
  initPeer = (e, t, s) => {
    const i = new Peer({ initiator: e, trickle: t, config: s }),
      r = (e) => i.__earlyDataBuffer.push(e);
    return (
      i.on(events.data, r),
      (i.__earlyDataBuffer = []),
      (i.__drainEarlyData = (e) => {
        i.off(events.data, r),
          i.__earlyDataBuffer.forEach(e),
          delete i.__earlyDataBuffer,
          delete i.__drainEarlyData;
      }),
      i
    );
  },
  genId = (e) =>
    Array(e)
      .fill()
      .map(() => charSet[Math.floor(62 * Math.random())])
      .join(""),
  initGuard = (e, t) => (s, i) => {
    if (e[i]) return e[i];
    if (!s) throw mkErr("requires a config map as the first argument");
    if (!s.appId && !s.firebaseApp)
      throw mkErr("config map is missing appId field");
    if (!i) throw mkErr("namespace argument required");
    return (e[i] = t(s, i));
  },
  libName = "Trystero",
  selfId = genId(20),
  ...
  noOp = () => {},
  mkErr = (e) => new Error(`${libName}: ${e}`),
  encodeBytes = (e) => new TextEncoder().encode(e),
  decodeBytes = (e) => new TextDecoder().decode(e),
  events = fromEntries(
  ...
  combineChunks = (e) => {
    const t = new Uint8Array(e.reduce((e, t) => e + t.byteLength, 0));
    return e.reduce((e, s) => (t.set(s, e), e + s.byteLength), 0), t;
  },
...
var room = (e, t) => {
  const s = {},
    ...
    a = {},
    c = (e, t) =>
      (e ? (Array.isArray(e) ? e : [e]) : keys(s)).flatMap((e) => {
        const i = s[e];
        return i
          ? t(e, i)
          : (console.warn(`${libName}: no peer with id ${e} found`), []);
      }),
    d = (e) => {
      if (i[e]) return [i[e].send, i[e].setOnComplete, i[e].setOnProgress];
      if (!e) throw mkErr("action type argument is required");
      const t = encodeBytes(e);
      if (t.byteLength > 12)
        throw mkErr(
          `action type string "${e}" (${t.byteLength}b) exceeds byte limit (12). Hint: choose a shorter name.`
        );
      ...
      return (
        (i[e] = {
          ...
          send: async (e, t, i, o) => {
            ...
            if (void 0 === e) throw mkErr("action data cannot be undefined");
            const a = "string" != typeof e,
              d = e instanceof Blob,
              h = d || e instanceof ArrayBuffer || e instanceof TypedArray;
            if (i && !h)
              throw mkErr(
                "action meta argument can only be used with binary data"
              );
            const l = h
                ? new Uint8Array(d ? await e.arrayBuffer() : e)
                : encodeBytes(a ? JSON.stringify(e) : e),
              _ = i ? encodeBytes(JSON.stringify(i)) : null,
              p = Math.ceil(l.byteLength / 16369) + (i ? 1 : 0),
              u = new Array(p).fill().map((e, t) => {
                const s = t === p - 1,
                  o = i && 0 === t,
                  c = new Uint8Array(
                    15 +
                      (o
                        ? _.byteLength
                        : s
                        ? l.byteLength - 16369 * (p - (i ? 2 : 1))
                        : 16369)
                  );
                return (
                  c.set(r),
                  c.set([n], 12),
                  c.set([s | (o << 1) | (h << 2) | (a << 3)], 13),
                  c.set([Math.round(((t + 1) / p) * 255)], 14),
                  c.set(
                    i
                      ? o
                        ? _
                        : l.subarray(16369 * (t - 1), 16369 * t)
                      : l.subarray(16369 * t, 16369 * (t + 1)),
                    15
                  ),
                  c
                );
                  ...
                  for (; n < p; ) {
                    ...
                    if (
                      (r.bufferedAmount > r.bufferedAmountLowThreshold &&
                        (await new Promise((e) => {
                          const t = () => {
                            r.removeEventListener(buffLowEvent, t), e();
                          };
                          r.addEventListener(buffLowEvent, t);
                        })),
                      !s[e])
                    )
                      break;
                    t.send(a), n++, o && o(a[14] / 255, e, i);
                  }
                })
              )
            );
    ...
    h = (e, t) => {
      const s = new Uint8Array(t),
        n = decodeBytes(s.subarray(0, 12)).replaceAll("\0", ""),
        [o] = s.subarray(12, 13),
        [a] = s.subarray(13, 14),
        [c] = s.subarray(14, 15),
        d = s.subarray(15),
        ...
        _ = !!(4 & a),
        p = !!(8 & a);
      if (!i[n]) throw mkErr(`received message with unregistered type (${n})`);
      r[e] || (r[e] = {}), r[e][n] || (r[e][n] = {});
      let u = r[e][n][o];
      if (
        (u || (u = r[e][n][o] = { chunks: [] }),
        l ? (u.meta = JSON.parse(decodeBytes(d))) : u.chunks.push(d),
        i[n].onProgress(c / 255, e, u.meta),
        !h)
      )
        return;
      const g = combineChunks(u.chunks);
      if (_) i[n].onComplete(g, e, u.meta);
      else {
        const t = decodeBytes(g);
        i[n].onComplete(p ? JSON.parse(t) : t, e);
      }
      delete r[e][n][o];
    },
    [l, _] = d("__91n6__"),
  ...
  return (
    e((e, t) => {
      if (s[t]) return;
      const i = h.bind(null, t);
      (s[t] = e),
        e.on(events.signal, (e) => g(e, t)),
        e.on(events.close, () =>
          ((e) => {
            s[e] && (delete s[e], delete r[e], delete n[e], w(e));
          })(t)
        ),
        e.on(events.data, i),
        e.on(events.stream, (e) => {
          R(e, t, o[t]), delete o[t];
        }),
        e.on(events.track, (e, s) => {
          T(e, s, t, a[t]), delete a[t];
        }),
        e.on(events.error, (e) => {
          "ERR_DATA_CHANNEL" !== e.code && console.error(e);
        }),
        b(t),
        e.__drainEarlyData(i);
    }),
    _((e, t) => p(null, t)),
    u((e, t) => {
      n[t] && (n[t](), delete n[t]);
    ...
    E((e, t) => (a[t] = e)),
    {
      makeAction: d,
      ping: async (e) => {
        if (!e) throw mkErr("ping() must be called with target peer ID");
        const t = Date.now();
        return l(null, e), await new Promise((t) => (n[e] = t)), Date.now() - t;
      },
      leave: () => {
        entries(s).forEach(([e, t]) => {
          t.destroy(), delete s[e];
        }),
          t();
      },
      getPeers: () => fromEntries(entries(s).map(([e, t]) => [e, t._pc])),
      addStream: (e, t, s) =>
        c(t, async (t, i) => {
          s && (await m(s, t)), i.addStream(e);
        }),
      removeStream: (e, t) => c(t, (t, s) => s.removeStream(e)),
      addTrack: (e, t, s, i) =>
        c(s, async (s, r) => {
          i && (await C(i, s)), r.addTrack(e, t);
        }),
      removeTrack: (e, t, s) => c(s, (s, i) => i.removeTrack(e, t)),
      replaceTrack: (e, t, s, i, r) =>
        c(i, async (i, n) => {
          r && (await C(r, i)), n.replaceTrack(e, t, s);
        }),
      onPeerJoin: (e) => (b = e),
      onPeerLeave: (e) => (w = e),
      onPeerStream: (e) => (R = e),
...
const algo = "AES-CBC",
  pack = (e) => btoa(String.fromCharCode.apply(null, new Uint8Array(e))),
  unpack = (e) => {
    const t = atob(e);
  ...
  genKey = async (e, t) =>
    crypto.subtle.importKey(
      "raw",
      await crypto.subtle.digest({ name: "SHA-256" }, encodeBytes(`${e}:${t}`)),
      { name: algo },
      !1,
      ["encrypt", "decrypt"]
    ),
  ...
  decrypt = async (e, t) => {
    const { c: s, iv: i } = JSON.parse(t);
    return decodeBytes(
      await crypto.subtle.decrypt(
        { name: algo, iv: new Uint8Array(i) },
        await e,
        unpack(s)
      )
    );
  ...
  joinRoom = initGuard(occupiedRooms, (e, t) => {
    const s = {},
      i = e.password && genKey(e.password, t),
      r = (e.trackerUrls || defaultTrackerUrls).slice(
        0,
        e.trackerUrls ? e.trackerUrls.length : e.trackerRedundancy || 2
      );
    if (!r.length) throw mkErr("trackerUrls is empty");
    const n = crypto.subtle
        .digest("SHA-1", encodeBytes(`${libName}:${e.appId}:${t}`))
        .then((e) =>
      ...
      o = (t) =>
        fromEntries(
          Array(t)
            .fill()
            .map(() => {
              const t = initPeer(!0, !1, e.rtcConfig);
              return [
                genId(20),
                {
                  peer: t,
                  offerP: new Promise((e) => t.once(events.signal, e)),
                },
              ];
            })
        ),
      a = async (t, r) => {
        ...
        try {
          ...
          return void console.error(`${libName}: received malformed SDP JSON`);
        }
        if (a.info_hash !== o || (a.peer_id && a.peer_id === selfId)) return;
        const c = a["failure reason"];
        if (c) console.warn(`${libName}: torrent tracker failure (${c})`);
        else {
          if (
            (a.interval &&
              a.interval > g &&
              ...
              (clearInterval(f),
              (g = a.interval),
              (f = setInterval(h, 1e3 * g))),
            a.offer && a.offer_id)
          ) {
            if (s[a.peer_id] || y[a.offer_id]) return;
            y[a.offer_id] = !0;
            const r = initPeer(!1, !1, e.rtcConfig);
            return (
              r.once(events.signal, async (e) =>
                t.send(
                  JSON.stringify({
                    answer: i ? { ...e, sdp: await encrypt(i, e.sdp) } : e,
                    action: "announce",
                    info_hash: o,
                    peer_id: selfId,
              ...
              r.on(events.connect, () => _(r, a.peer_id)),
              r.on(events.close, () => p(r, a.peer_id, a.offer_id)),
              void r.signal(
                i ? { ...a.offer, sdp: await decrypt(i, a.offer.sdp) } : a.offer
              )
            );
          }
          if (a.answer) {
            if (s[a.peer_id] || y[a.offer_id]) return;
            const e = u[a.offer_id];
            if (e) {
              const { peer: t } = e;
              if (t.destroyed) return;
              (y[a.offer_id] = !0),
                t.on(events.connect, () => _(t, a.peer_id, a.offer_id)),
                t.on(events.close, () => p(t, a.peer_id, a.offer_id)),
                t.signal(
                  i
                    ? { ...a.answer, sdp: await decrypt(i, a.answer.sdp) }
                    : a.answer
                );
            }
          }
        ...
        e.send(
          JSON.stringify({
            ...
            offers: await Promise.all(
              entries(u).map(async ([e, { offerP: t }]) => {
                const s = await t;
                return {
                  offer_id: e,
                  offer: i ? { ...s, sdp: await encrypt(i, s.sdp) } : s,
                };
              })
            ),
          })
      ...
      d = (e, t, s) => (
        s || !socketPromises[e]
          ? ((socketListeners[e] = { ...socketListeners[e], [t]: a }),
            (socketPromises[e] = new Promise((s) => {
              const i = new WebSocket(e);
              (sockets[e] = i),
                i.addEventListener("open", () => {
                  (socketRetryTimeouts[e] = 4e3), s(i);
                }),
                i.addEventListener("message", (t) =>
                  values(socketListeners[e]).forEach((e) => e(i, t))
                ),
                i.addEventListener("close", async () => {
                  (socketRetryTimeouts[e] = socketRetryTimeouts[e] ?? 4e3),
                    await sleep(socketRetryTimeouts[e]),
                    (socketRetryTimeouts[e] *= 2),
                    d(e, t, !0);
                });
            })))
          : (socketListeners[e][t] = a),
        socketPromises[e]
      ),
      h = async () => {
        const e = await n;
      ...
      p = (e, t, i) => {
        delete s[t], e.destroy();
        i in u && (delete u[i], (u = { ...u, ...o(1) }));
      };
    let u,
      g = 33,
      f = setInterval(h, 1e3 * g),
    ...
    return (
      ...
      room(
        ...
        async () => {
          const e = await n;
          r.forEach((t) => delete socketListeners[t][e]),
            delete occupiedRooms[t],
            clearInterval(f),
            l();
        }
      )
  ...
  getTrackers = () => ({ ...sockets });
export { getTrackers, joinRoom, selfId };
